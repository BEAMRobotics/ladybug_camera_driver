...
void LadybugCameraNodelet::onInit() {
  ...
  nh_priv.param<std::string>("data_capture_format", data_capture_format_,
                             "raw8");
  nh_priv.param<std::string>("data_capture_format", data_capture_format_,
                             "raw8");
  nh_priv.param<std::string>("color_processing_method",
                             color_processing_method_, "downsample16");
  nh_priv.param<std::string>("processed_pixel_format", processed_pixel_format_,
                             "bgr");
  nh_priv.param<bool>("publish_raw_images", publish_raw_images_, true);
  nh_priv.param<std::string>("frame_id", frame_id_, "camera");
  nh_priv.param<double>("diagnostics_desired_freq", diagnostics_desired_freq_,
                        10);
  nh_priv.param<double>("diagnostics_min_freq", diagnostics_min_freq_,
                        diagnostics_desired_freq_);
  nh_priv.param<double>("diagnostics_max_freq", diagnostics_max_freq_,
                        diagnostics_desired_freq_);
  nh_priv.param<double>("diagnostics_freq_tolerance",
                        diagnostics_freq_tolerance_, 0.1);
  nh_priv.param<int>("diagnostics_window_size", diagnostics_window_size_, 10);
  nh_priv.param<double>("diagnostics_min_acceptable_delay",
                        diagnostics_min_acceptable_, 0.0);
  nh_priv.param<double>("diagnostics_max_acceptable_delay",
                        diagnostics_max_acceptable_, 0.01);

  // Do not call the ConnectCallback function until after we are done
  // initializing.
  boost::mutex::scoped_lock scoped_lock(connect_mutex_);

  // Initialize the dynamic reconfigure server / bind the ConfigCallback
  reconfig_srv_ = std::make_shared<
      dynamic_reconfigure::Server<ladybug_camera_driver::LadybugConfig> >(
      config_mutex_, nh_priv);
  dynamic_reconfigure::Server<
      ladybug_camera_driver::LadybugConfig>::CallbackType config_callback =
      boost::bind(&ladybug_camera_driver::LadybugCameraNodelet::ConfigCallback,
                  this, _1, _2);
  ...
  // Initialize image transport / image transport SubscriberStatus callback (for
  // monitoring subscriptions)
  transport_.reset(new image_transport::ImageTransport(nh));
  image_transport::SubscriberStatusCallback transport_sub_cb =
      boost::bind(&LadybugCameraNodelet::ConnectCallback, this);

  for (unsigned int i = 0; i < LADYBUG_NUM_CAMERAS; i++) {
    const std::string istr = std::to_string(i);
    const std::string topic =
        std::string("/ladybug/camera_") + istr + std::string("/image_raw");
    transport_pub_[i] = transport_->advertiseCamera(topic, 5, transport_sub_cb,
                                                    transport_sub_cb);
  }
  ...
  ros::SubscriberStatusCallback tiles_sub_cb =
      boost::bind(&LadybugCameraNodelet::ConnectCallback, this);

  // Set up a diagnostics publisher (clang-format messing up formating)
  tiles_publisher_.reset(
      new diagnostic_updater::DiagnosedPublisher<ladybug_msgs::LadybugTiles>(
          nh.advertise<ladybug_msgs::LadybugTiles>("image_tiles", 5,
                                                   tiles_sub_cb, tiles_sub_cb),
          updater_,
          diagnostic_updater::FrequencyStatusParam(
              &diagnostics_min_freq_, &diagnostics_max_freq_,
              diagnostics_freq_tolerance_, diagnostics_window_size_),
          diagnostic_updater::TimeStampStatusParam(
              diagnostics_min_acceptable_, diagnostics_max_acceptable_)));

  /*
  jpeg_publisher_.reset(new
  diagnostic_updater::DiagnosedPublisher<sensor_msgs::CompressedImage>(nh.advertise<sensor_msgs::CompressedImage>("image_test/compressed",
  5, tiles_sub_cb, tiles_sub_cb), updater_,
             diagnostic_updater::FrequencyStatusParam(&diagnostics_min_freq_,
  &diagnostics_max_freq_, freq_tolerance, window_size),
  ...
  */
  ...
  ros::SubscriberStatusCallback diag_cb =
      boost::bind(&LadybugCameraNodelet::DiagnosticsCallback, this);
  diagnostics_pub_.reset(
      new ros::Publisher(nh.advertise<diagnostic_msgs::DiagnosticArray>(
          "/diagnostics", 1, diag_cb, diag_cb)));

  diagnostics_manager_ =
      std::unique_ptr<DiagnosticsManager>(new DiagnosticsManager(
          frame_id_, std::to_string(ladybug_->getSerial()), diagnostics_pub_));

  // TODO: Add the diagnostics available for the ladybug (pressure, temperature)
  /*diagnostics_manager_->addDiagnostic("DeviceTemperature", true,
    std::make_pair(0.0f, 90.0f), -10.0f, 95.0f);
    diagnostics_manager_->addDiagnostic("AcquisitionResultingFrameRate", true,
    std::make_pair(10.0f, 60.0f), 5.0f, 90.0f);
    diagnostics_manager_->addDiagnostic("PowerSupplyVoltage", true,
    std::make_pair(4.5f, 5.2f), 4.4f, 5.3f);
    diagnostics_manager_->addDiagnostic("PowerSupplyCurrent", true,
...
void LadybugCameraNodelet::ConfigCallback(
    ladybug_camera_driver::LadybugConfig& config, uint32_t level) {
...
void LadybugCameraNodelet::DiagnosticsCallback() {
  if (!diagnostics_thread_) // Can't publish diagnostics if we aren't connected
  {
    ...
    diagnostics_thread_.reset(new boost::thread(boost::bind(
        &ladybug_camera_driver::LadybugCameraNodelet::DiagnosticsPoll, this)));
  }
}
...
void LadybugCameraNodelet::ConnectCallback() {
  ...
  // Grab the connect_mutex. Wait until we're done initializing before letting
  // this function through.
  boost::mutex::scoped_lock scoped_lock(connect_mutex_);

  // Check if we should disconnect (there are 0 subscribers to our data)
  if (transport_pub_[0].getNumSubscribers() == 0 &&
      tiles_publisher_->getPublisher().getNumSubscribers() == 0) {
  ...
  {
    // Start the thread to loop through and publish messages
    publish_thread_.reset(new boost::thread(boost::bind(
        &ladybug_camera_driver::LadybugCameraNodelet::DevicePoll, this)));
  } else {
    ...
    switch (state) {
      ...
      case DISCONNECTED:
        ...
        try {
          ...
          NODELET_ERROR_COND(state_changed, "Failed to connect with error: %s",
                             e.what());
          ros::Duration(1.0).sleep(); // sleep for one second each time
        }
      ...
      case STARTED:
        try {
          ...
          ladybug_msgs::LadybugTilesPtr tile_images =
              boost::make_shared<ladybug_msgs::LadybugTiles>();
          ladybug_->GrabImage(tile_images, frame_id_);

          // Loop over each of the (6) ladybug cameras
          for (unsigned int i = 0; i < 6; i++) {
            camera_info_[i].reset(new sensor_msgs::CameraInfo(
                camera_info_manager_->getCameraInfo()));
            camera_info_[i]->header.stamp = tile_images->header.stamp;
            camera_info_[i]->header.frame_id =
                tile_images->images[i].header.frame_id;
            ...
            transport_pub_[i].publish(
                boost::make_shared<sensor_msgs::Image>(tile_images->images[i]),
                camera_info_[i]);
          }

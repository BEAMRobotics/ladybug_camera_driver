...
private:
  /**
   * Function called when nodelet is loaded (serves as a psuedo-constructor for
   * the LadybugNodelet class). This function needs to do the MINIMUM amount of
   * work to get the nodelet running. Nodelets should not call blocking
   * functions here. Grabs ROS parameters, initializes dynamic reconfigure,
   * initializes camera info manager,
   */
   ...
   * Values will be changed to those the driver is currently using.
   * @param level driver_base reconfiguration level. See
   * driver_base/SensorLevels.h for more information.
   */
  void ConfigCallback(ladybug_camera_driver::LadybugConfig& config,
                      uint32_t level);
  ...
  /**
   * Callback for when the subscriber status for one of the relevant topics
   * changes. Case 1: No subscribers, Connected Case 2: No subscribers,
   * Disconnected
   */
  ...
  std::unique_ptr<DiagnosticsManager>
      diagnostics_manager_; // Manager for diagnostics
  diagnostic_updater::Updater
      updater_; // Handles publishing diagnostics messages.
  std::shared_ptr<ros::Publisher>
      diagnostics_pub_; // Publisher for publishing camera diagnostics
  ...
  boost::shared_ptr<boost::thread>
      publish_thread_; // The thread that reads and publishes the images.
  boost::shared_ptr<boost::thread>
      diagnostics_thread_; // The thread that reads and publishes the
                           // diagnostics.

  //! Group image publishing
  bool publish_raw_images_ = true; // Boolean for whether we should grab raw
                                   // images (vs. compressed) [Rosparam].
  std::string frame_id_ =
      "camera"; // Frame id for the camera messages [Rosparam].

  std::shared_ptr<image_transport::ImageTransport>
      transport_; // Needed to initialize and keep the ImageTransport in scope.
  image_transport::CameraPublisher
      transport_pub_[6]; // 6 camera publishers corresponding to each of the
                         // ladybug cameras

  // Diagnosed publishers, need to be a pointer because of constructor
  // requirements
  std::shared_ptr<
      diagnostic_updater::DiagnosedPublisher<ladybug_msgs::LadybugTiles> >
      tiles_publisher_;
  std::shared_ptr<
      diagnostic_updater::DiagnosedPublisher<sensor_msgs::CompressedImage> >
      jpeg_publisher_;
  ...
  double diagnostics_desired_freq_ =
      10; // Desired frequency for publishing camera diagnostics.
  double diagnostics_min_freq_ =
      diagnostics_desired_freq_; // Minimum publish frequency for publishing
                                 // camera diagnostics.
  double diagnostics_max_freq_ =
      diagnostics_desired_freq_; // Maximum publish frequency for publishing
                                 // camera diagnostics.
  double diagnostics_freq_tolerance_ =
      0.1; // Tolerance before stating error on publish frequency, fraction of
           // desired.
  int diagnostics_window_size_ =
      10; // Number of samples to consider in frequency
  double diagnostics_min_acceptable_ =
      0; // The minimum publishing delay (in seconds) before warning.  Negative
         // values mean future dated messages.
  double diagnostics_max_acceptable_ =
      0.01; // The maximum publishing delay (in seconds) before warning.
...
PLUGINLIB_EXPORT_CLASS(ladybug_camera_driver::LadybugCameraNodelet,
